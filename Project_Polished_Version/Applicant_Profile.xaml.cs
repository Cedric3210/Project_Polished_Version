...
namespace Project_Polished_Version.Classes
{
    public partial class Applicant_Profile : Window
    {
        ...
        public Applicant_Profile()
        {
            InitializeComponent();
            ShowInfo();
            //_ = getAboutDataASYNC(); // Avoid using 'await' in constructors
        }
        ...
        private void disableButton()
        {
            using (MySqlConnection connection = new MySqlConnection(
                "Server=localhost;Database=project_database;User=root;Password=Cedric1234%%"))
            {
                try
                {
                    connection.Open();
        ...
        private async Task<string> getAboutDataASYNC()
        {
            ...
            using (MySqlConnection connection = new MySqlConnection(connectionString))
            {
                try
                {
                    ...
                    string query = "SELECT content FROM applicant_info WHERE userId = @userId;";
